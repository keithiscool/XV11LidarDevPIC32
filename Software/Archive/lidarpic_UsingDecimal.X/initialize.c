...
//Control the speed the lidar rotates (alter speed to 4000 to achieve around 200rpm)
//Fosc==80MHz
void PWM(void){
    T2CONbits.ON = 0;
    T2CONbits.T32 = 0; //16 bit mode
    T2CONbits.TCKPS = 0b001; //prescalar of 2
    T2CONbits.TGATE = 0; // setting 200rpm for testing with python
    PR2 = 4000; //4000 is around 200rpm
