...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
                ...
                for (i = 0; i < 4; i++) {
                    if (!InvalidFlag[i]) { // the data is valid within the present 22byte packet
                        ...
                        //possibly compare the previous and present set of data later...
                        ...
                        if(DistanceArr[DegreeIndex+i] > 1) { // check if polar distance is useful data
                        ...
                        //check if enough angles are retreved and goodData (if so, continue in main.c code)
                        if(AnglesCoveredTotal < 180) {
                            AnglesCoveredTotal++;
                        }
                        if(AnglesCoveredTotal > enoughAnglesThreshold) {
                            enoughAngles = true;
                        }
                    ...
                    if(DistanceArr[DegreeIndex+i] > 1) { //only return degrees if the associated distance is valid
...
bool debugLidarPolarData(void) {
    ...
    if(LIDARdecode(myCurrentDegrees) == true) { //88 is rotation offset for lidar (rotate clockwise if positive)
...
//            printf("\x1b[H"); //clear screen // ANSI Terminal code: (ESC[H == home) & (ESC[2J == clear screen) // THIS IS FORMATTING FOR ANSI (DOES NOT WORK WITH DMA!)
            ...
            for(i=0;i<181;i++) {
                if(U6STAbits.UTXBF == 0) { //check to see if the UART buffer is not full - if it is not full, send debug data out UART1
                    if ( (i % (PRINT_NUM_PER_LINE/2) ) == 0)  //print 4 x,y distances per line
